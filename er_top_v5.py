...
import os
os.chdir('/Users/farismismar/Desktop/E_Projects/UT Austin Ph.D. EE/Courses/4- Spring 2017/Data Mining/Project/2- HR Employee Retention/xtr0')
...
dataset = pd.read_csv('../Datasets/HR_comma_sep.csv')
df = dataset.drop(['sales', 'salary', 'left'], axis=1)
df.columns = ['satisfaction level', 'last eval', 'num project', 'avg monthly hrs', 'time spent company', 'work accident', 'promotion last 5yrs']

from biokit.viz import corrplot
c = corrplot.Corrplot(df)
c.plot(colorbar=True, cmap='jet', method='color')
corrplot.plt.savefig('corrplot.pdf', format='pdf')
...
data_summary = dataset['left'].value_counts()
...
#train['Outcome'] = train['Outcome'].replace(['Active', 'Inactive'], [1, 0])
#test['Outcome'] = test['Outcome'].replace(['Active', 'Inactive'], [1, 0])


X_train = train.drop('left', axis = 1)
X_test = test.drop('left', axis = 1)

y_train = train['left'].values
y_test = test['left'].values
...
try:
    ...
    xtr6 = train6.drop('left', axis = 1)
    ytr6 = train6['left']
except:
    print('Undersampling 50% failed')
    pass
